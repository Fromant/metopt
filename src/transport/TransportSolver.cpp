...
static std::vector<std::vector<double>> extractOriginalPlan(const std::vector<std::vector<double>>& expanded_plan,
...
TransportSolution TransportSolver::solveNorthwestCorner(const TransportProblem& problem, bool verbose) {
    ...
    log(std::format("Метод СЗУ: {} реальных поставщиков, {} реальных потребителей", real_m, real_n), &solution.log,
        verbose);
    ...
    while (i < real_m && j < real_n) {
        ...
        log(std::format("  x[{}][{}] = {} (остаток A{}: {:.2f}, B{}: {:.2f})", i + 1, j + 1, shipment, i + 1,
    ...
    if (has_dummy_consumer) {
        for (i = 0; i < real_m; ++i) {
            if (avail[i] > 1e-9) {
                ...
                log(std::format("  x[{}][{}] = {} (фиктивный потребитель)", i + 1, real_n + 1, avail[i]), &solution.log,
                    verbose);
                avail[i] = 0;
            }
        }
    ...
    if (has_dummy_supplier) {
        ...
        for (j = 0; j < real_n; ++j) {
            if (need[j] > 1e-9 && avail[dummy_i] > 1e-9) {
                ...
                log(std::format("  x[{}][{}] = {} (ФИКТИВНЫЙ = недопоставка)", dummy_i + 1, j + 1, shipment),
                    &solution.log, verbose);
    ...
    if (has_dummy_supplier) {
        ...
        if (!connected_to_real) {
            ...
            log(std::format("  [СВЯЗНОСТЬ] Добавлена вырожденная x[{}][{}] = 0 (penalty={:.2f})", dummy_i + 1,
                            best_j + 1, min_penalty),
                &solution.log, verbose);
        }
    }
        ...
        for (size_t ii = 0; ii < m && solution.countBasicVars() < expected_basis; ++ii) {
            for (size_t jj = 0; jj < n && solution.countBasicVars() < expected_basis; ++jj) {
                if (!solution.is_basic[ii][jj]) {
                    ...
                    log(std::format("  [Вырожденность] Добавлена нулевая базисная x[{}][{}] = 0", ii + 1, jj + 1),
                        &solution.log, verbose);
                    break;
                }
            }
    ...
    log(std::format("СЗУ завершено: базисных = {} (ожидалось {})", solution.countBasicVars(), expected_basis),
        &solution.log, verbose);
...
bool TransportSolver::calculatePotentials(const TransportProblem& problem, const TransportSolution& solution,
        ...
        for (size_t i = 0; i < m; ++i) {
            for (size_t j = 0; j < n; ++j) {
                if (solution.is_basic[i][j]) {
                    std::cout << "    (" << i + 1 << "," << j + 1 << "): c=" << problem.costs()[i][j] << "\n";
                }
            }
        }
    }
    ...
    for (size_t i = 0; i < m; ++i)
        if (u[i] > 1e19) {
            if (verbose)
                std::cout << "  [WARN] Не вычислен u[" << i << "]\n";
            all_computed = false;
        }
    for (size_t j = 0; j < n; ++j)
        if (v[j] > 1e19) {
            if (verbose)
                std::cout << "  [WARN] Не вычислен v[" << j << "]\n";
            all_computed = false;
        }

    if (!all_computed) {
        if (verbose)
            std::cout << "  [ERROR] Базис несвязный!\n";
        return false;
    }
...
bool TransportSolver::findImprovingCell(const TransportProblem& problem, const std::vector<double>& u,
    ...
    if (found && verbose) {
        if (out_is_penalty) {
            log(std::format("Найдена улучшающая ячейка штрафа (A{},B{}): Δ = {:.3f}", out_i + 1, out_j + 1, out_delta),
                nullptr, verbose);
        } else {
...
bool TransportSolver::findCycle(const TransportSolution& solution, size_t start_i, size_t start_j,
    ...
    std::function<bool(size_t, size_t, bool)> dfs = [&](size_t i, size_t j, bool last_horizontal) -> bool {
    ...
    for (size_t jj : basic_in_row[start_i]) {
        if (jj == start_j)
            continue;
        if (dfs(start_i, jj, true))
            return true;
    }
    for (size_t ii : basic_in_col[start_j]) {
        if (ii == start_i)
            continue;
        if (dfs(ii, start_j, false))
            return true;
    }
...
double TransportSolver::redistributeAlongCycle(TransportSolution& solution,
    ...
    for (size_t k = 0; k < cycle.size() - 1; ++k) {
        ...
        if (add) {
            ...
            if (solution.shipments[i][j] < 0)
                solution.shipments[i][j] = 0; // защита
        }
        add = !add;
    }
    ...
    if (verbose) {
        std::cout << "  Theta = " << std::fixed << std::setprecision(3) << theta << ", exiting cell: ("
                  << exiting_cell.first + 1 << "," << exiting_cell.second + 1 << ")\n";
    }
...
TransportSolution TransportSolver::solvePotentials(const TransportProblem& problem,
    ...
    while (solution.iterations < MAX_TOTAL_ITER) {
        ...
        if (!findImprovingCell(problem, u, v, solution, imp_i, imp_j, delta, is_penalty_entering, verbose)) {
        ...
        if (current_basis < m + n - 1) {
            for (size_t i = 0; i < m && solution.countBasicVars() < m + n - 1; ++i) {
                for (size_t j = 0; j < n && solution.countBasicVars() < m + n - 1; ++j) {
                    if (!solution.is_basic[i][j]) {
                        solution.is_basic[i][j] = true;
                        log(std::format("  [Вырожденность] Добавлена нулевая базисная x[{}][{}] = 0", i + 1, j + 1),
                            &solution.log, verbose);
                        break;
                    }
                }
...
TransportSolution TransportSolver::solve(const TransportProblem& problem, bool verbose) {
    ...
    if (verbose && problem.hasPenalties()) {
        ...
        std::cout << "       Поставщиков: " << expanded.numSuppliers() << " (оригинальных: " << problem.numSuppliers()
                  << ")\n";
        std::cout << "       Потребителей: " << expanded.numConsumers() << " (оригинальных: " << problem.numConsumers()
                  << ")\n";
    }
    ...
    original_solution.shipments =
        extractOriginalPlan(expanded_solution.shipments, problem.numSuppliers(), problem.numConsumers());

    original_solution.is_basic.assign(problem.numSuppliers(), std::vector<bool>(problem.numConsumers(), false));
    ...
    while (original_solution.countBasicVars() < expected_basis) {
        for (size_t i = 0; i < problem.numSuppliers() && original_solution.countBasicVars() < expected_basis; ++i) {
            for (size_t j = 0; j < problem.numConsumers() && original_solution.countBasicVars() < expected_basis; ++j) {
    ...
    for (size_t i = 0; i < problem.numSuppliers(); ++i) {
        for (size_t j = 0; j < problem.numConsumers(); ++j) {
            original_solution.transportation_cost += problem.costs()[i][j] * original_solution.shipments[i][j];
        }
    }
    ...
    if (problem.hasPenalties()) {
        ...
        for (size_t j = 0; j < problem.numConsumers(); ++j) {
            original_solution.penalty_cost += expanded_solution.shipments[dummy_idx][j] * problem.penaltyRates()->at(j);
        }
    }
